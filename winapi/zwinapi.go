// go build mksyscall_windows.go && ./mksyscall_windows winapi.go font.go
...
func GetWindowTextLength(hwnd syscall.Handle) (len int32, err error) {
	r0, _, e1 := syscall.Syscall(procGetWindowTextLengthW.Addr(), 1, uintptr(hwnd), 0, 0)
	len = int32(r0)
	if len == 0 {
		if e1 != 0 {
			err = error(e1)
...
func SetWindowText(hwnd syscall.Handle, str *uint16) (err error) {
	r1, _, e1 := syscall.Syscall(procSetWindowTextW.Addr(), 2, uintptr(hwnd), uintptr(unsafe.Pointer(str)), 0)
	if r1 == 0 {
		if e1 != 0 {
			err = error(e1)
		} else {
...
func EnableWindow(hwnd syscall.Handle, enable bool) (wasenabled bool) {
	var _p0 uint32
	if enable {
		_p0 = 1
	} else {
...
func IsWindowEnabled(hwnd syscall.Handle) (enabled bool) {
...
func SetWindowSubclass(hwnd syscall.Handle, fn uintptr, id uintptr, refData *uint32) (err error) {
	r1, _, e1 := syscall.Syscall6(procSetWindowSubclass.Addr(), 4, uintptr(hwnd), uintptr(fn), uintptr(id), uintptr(unsafe.Pointer(refData)), 0, 0)
	if r1 == 0 {
		if e1 != 0 {
			err = error(e1)
		} else {
...
func DefSubclassProc(hwnd syscall.Handle, msg uint32, wparam uintptr, lparam uintptr) (lresult uintptr) {
	r0, _, _ := syscall.Syscall6(procDefSubclassProc.Addr(), 4, uintptr(hwnd), uintptr(msg), uintptr(wparam), uintptr(lparam), 0, 0)
	lresult = uintptr(r0)
	return
}

func RemoveWindowSubclass(hwnd syscall.Handle, fn uintptr, id uintptr) (err error) {
	r1, _, e1 := syscall.Syscall(procRemoveWindowSubclass.Addr(), 3, uintptr(hwnd), uintptr(fn), uintptr(id))
	if r1 == 0 {
		if e1 != 0 {
			err = error(e1)
		} else {
